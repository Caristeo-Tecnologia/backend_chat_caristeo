...
async function handleAsaas() {
    logger_1.logger.info("Iniciando integração com a Asaas");
    const job = new CronJob("*/5 * * * * *", async () => {
        const companies = await Company_1.default.findAll({
            where: {
                asaasSubscriptionId: { [sequelize_1.Op.is]: null },
                cpfCnpj: { [sequelize_1.Op.ne]: null }
            }
        });
        companies.forEach(async (company) => {
            logger_1.logger.info(`Criando assinatura para a compania: ${company.id}`);
            await (0, CreateAsaasSubscriptionService_1.default)(company.id);
        });
    });
    job.start();
}
