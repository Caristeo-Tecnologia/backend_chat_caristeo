...
const verifyMessage = async (msg, ticket, contact, fromQueue, hasOption) => {
    const io = (0, socket_1.getIO)();
    const quotedMsg = await verifyQuotedMessage(msg);
    const body = (0, exports.getBodyMessage)(msg);
...
const verifyQueue = async (wbot, msg, ticket, contact, mediaSent) => {
    ...
    const botText = async () => {
        ...
        await (0, exports.verifyMessage)(sendMsg, ticket, ticket.contact, fromQueue, hastOption);
    };
    if (choosenQueue) {
        let chatbot = false;
...
const handleChartbot = async (ticket, msg, wbot, dontReadTheFirstQuestion = false) => {
    ...
    if (messageBody == "#") {
        ...
        await (0, exports.verifyMessage)(msg, ticket, ticket.contact, true, true);
        await verifyQueue(wbot, msg, ticket, ticket.contact);
        return;
    }
    if (messageBody == "x" || messageBody == "X") {
        (0, UpdateTicketService_1.default)({ ticketData: { status: "closed" }, ticketId: ticket.id, companyId: ticket.companyId });
        return;
    }
    ...
    if (!(0, lodash_1.isNil)(queue) && (0, lodash_1.isNil)(ticket.queueOptionId)) {
        ...
        const botText = async () => {
            ...
            const fromQueue = true;
            await (0, exports.verifyMessage)(sendMsg, ticket, ticket.contact, fromQueue, hasOption);
        };
...
const handleMessage = async (msg, wbot, companyId) => {
    ...
    try {
        ...
        // voltar para o menu inicial
        if (bodyMessage == "#") {
            logger_1.logger.info(`[bodyMessage]`);
            await ticket.update({
                queueOptionId: null,
                chatbot: false,
                queueId: null
            });
            await (0, exports.verifyMessage)(msg, ticket, contact, true, true);
            await verifyQueue(wbot, msg, ticket, ticket.contact);
            return;
        }
