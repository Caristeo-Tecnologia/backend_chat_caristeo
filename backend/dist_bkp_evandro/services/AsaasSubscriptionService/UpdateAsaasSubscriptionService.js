...
const UpdateAsaasSubscriptionService = async (companyId, creditCard) => {
    ...
    if (subscription?.length) {
        ...
        const res = await axios_1.default.put(`${process.env.ASAAS_URL}/v3/subscriptions/${subscription[0].id}`, body, {
            headers
        });
    ...
    // ToDo: tratar resposta
    const res = await axios_1.default.post(`${process.env.ASAAS_URL}/v3/subscriptions`, body, {
        headers
    });
    invoices.forEach(async (invoice) => {
        await (0, DeleteAsaasPaymentsService_1.default)(invoice.asaasPaymentId);
